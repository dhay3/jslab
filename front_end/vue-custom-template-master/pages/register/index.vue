<template>
  <div class="main">
    ...
    <div class="sign-up-container">
      <el-form ref="userForm" :model="MemberRegisterVo">

        <el-form-item class="input-prepend restyle" prop="nickname" :rules="[{ required: true, message: '请输入你的昵称', trigger: 'blur' }]">
          <div>
            <el-input type="text" placeholder="你的昵称" v-model="MemberRegisterVo.nickname"/>
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="mobile" :rules="[{ required: true, message: '请输入手机号码', trigger: 'blur' },{validator: checkPhone, trigger: 'blur'}]">
          <div>
            <el-input type="text" placeholder="手机号" v-model="MemberRegisterVo.mobile"/>
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="code" :rules="[{ required: true, message: '请输入验证码', trigger: 'blur' }]">
          <div style="width: 100%;display: block;float: left;position: relative">
            <el-input type="text" placeholder="验证码" v-model="MemberRegisterVo.code"/>
            <i class="iconfont icon-phone"/>
          </div>
        ...
        <el-form-item class="input-prepend" prop="password" :rules="[{ required: true, message: '请输入密码', trigger: 'blur' }]">
          <div>
            <el-input type="password" placeholder="设置密码" v-model="MemberRegisterVo.password"/>
    ...
    methods: {
      getCodeFun() {
        ...
        this.$refs.userForm.validateField('mobile', (errMsg) => {
          if (errMsg == '') {
            register.sendCode(this.MemberRegisterVo.mobile).then(res => {
              this.sending = false;
              this.timeDown();
            });
          }
      ...
      submitRegister() {
        this.$refs['userForm'].validate((valid) => {
          if (valid) {
            register.registerMember(this.MemberRegisterVo).then(response => {
              //提示注册成功
              this.$message({
                type: 'success',
                message: "注册成功"
      ...
      checkPhone (rule, value, callback) {
        //debugger, {}表示匹配的次数,[]表示可以是什么数字,^匹配开头
